...
## 2026-08-24 (Day 10, main task) — Multi-seed evaluation: accuracy is stable, revenue is not

Chose multi-seed evaluation over the minimal-dashboard alternative (see chat transcript for the reasoning). Full writeup and numbers in **`docs/multi_seed_eval.md`** — headline finding: classifier accuracy across 5 real-Groq seeds is tight (mean 99.69%, stdev 0.70%, range [98.44%, 100%]), safe to quote as a stable claim. Simulated recovered revenue is **not** stable (mean INR 15,511.57, stdev INR 7,489.26 — 48.3% relative stdev, ~4.6x spread between the luckiest and unluckiest seed) — driven by retry-plan-count variance (16–32 across seeds) at this batch size. Day 8's single-run revenue figure (INR 8,047.30) should not be quoted as if representative on its own; a range is the honest claim. Also: `retry_exhausted_nudge` fired zero times across all 5 seeds (225+350 total events) — not a bug, just structurally rare at this scale (needs low classifier confidence, negative-EV plans, or non-structural all-vetoed cases, none common in a random batch).
